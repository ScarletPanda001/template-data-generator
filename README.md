...
A template for creating synthetic reasoning task data generators. Fork this repository and customize it to generate your own task data (physics simulations, mazes, sudoku, rotations, board games, etc.). The current implementation generates gravity motion prediction tasks.
...
class TaskConfig(GenerationConfig):
    ...
    # 1. Override defaults
    domain: str = Field(default="gravity")  # Task domain name
    ...
    gravity_acceleration: Optional[float] = Field(default=None, description="Gravity acceleration (m/s²)")
    ball_radius: int = Field(default=20, description="Ball radius in pixels")
    ball_color: tuple[int, int, int] = Field(default=(255, 0, 0), description="Ball color (RGB)")
    simulation_duration: float = Field(default=2.0, description="Simulation duration in seconds")
```
...
- `task_id`: Unique task identifier (e.g., `"gravity_0001"`)
- `domain`: Task domain (e.g., `"gravity"`, `"maze"`)
...
### 1. Physics Simulations (e.g., Gravity Motion)

- Use kinematic equations to simulate object motion
- Render objects and physical indicators (arrows, labels)
- Generate smooth physics-based animations
- Support different environments (planets with different gravity)
...
3. Review example code (currently gravity motion task)
